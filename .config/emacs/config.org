...
** Emoji 🥹
#+begin_src emacs-lisp
  (when (display-graphic-p) (set-fontset-font t 'emoji (font-spec :family "Apple Color Emoji" :size 12) nil 'prepend))
#+end_src
...
#+begin_src emacs-lisp
  (when (and (fboundp 'sqlite-available-p) (sqlite-available-p))
    ...
    (use-package org-roam-ui
      ...
      ;; :init (setq org-roam-ui-browser-function #'browse-url)
      )
    )
#+end_src
...
#+begin_src emacs-lisp
  (use-package eglot
    ...
    :custom
    ;; It's here to fix a stupid formatting issue
    ;; when dtrt-indent is guessing a different indentation from lsp's default indent
    ;; happened especially when I'm editing c++ files
...
#+begin_src emacs-lisp
  (use-package eglot-booster
    ...
    :after eglot
    :config
    (when (version<= "30" emacs-version) (setq eglot-booster-io-only t))
    (eglot-booster-mode))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (use-package quickrun
    :bind (("C-<f5>" . quickrun)
           ("C-c X"  . quickrun))
    :config
    ;; specify c++ standard as default will use c++98
    (quickrun-add-command "c++/default"
      '((:command . "clang++")
        (:exec . ("%c -std=c++20 -fsanitize=address %o -o %e %s" "%e %a"))
        (:remove . ("%e")))
      :default "c++")
    )
#+end_src
