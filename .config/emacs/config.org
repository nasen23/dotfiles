  ...
  The padding pushes TEXT to the right edge of the mode-line."
    (if (and (>= emacs-major-version 29) (display-graphic-p))
        (let* ((len (string-pixel-width text))
               (space-prop
                `(space :align-to (- (+ right right-margin) (+ (,len) right-margin))))
                ...
                (let* ((spaces
                  ...
                  `(,fringe
                    ...
                    ,(if (display-graphic-p)
                       ...
                       " ")
                    ,(propertize " " 'display '(space :align-to 9))
                    (:eval (if (window-dedicated-p) "🚷 " ""))
                    (:eval (if buffer-read-only "🔒" ""))
                    (:eval (let ((file (buffer-file-name))) (if major-mode (concat (nerd-icons-icon-for file) " ") "")))
